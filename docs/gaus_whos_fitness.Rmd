...
Mothers (and more generally parents) can have strong effects on the phenotype of their offspring, above the effect of their shared genes. These effects of the maternal provided environment on offspring phenotype are generally known as maternal effects and, in quantitative genetics, refer specifically to the consistent effect of a mother across all her offspring. Maternal effects are ubiquitous (Moore et al 2019), occurring in a wide range of taxa, with the strongest impact on traits expressed in juveniles (Lindholm, Pick, Wilson & Reale, Moore).

From an evolutionary perspective maternal effects can impact the evolutionary potential of traits. 
Generally we think of the response of a trait to selection ($R$) to be dependent on the additive genetic variation ($V_A$) in that trait and the strength of selection, summarised by the breeders equation ($R=h^2S$) or gradient equation ($R=V_A\beta$).
Maternal effects have historically been seen as a nuisance parameter in the estimation of selection response (REFs), as not accounting for these shared effects across siblings can dramatically inflate estimates of $V_A$ (REFs - Kruuk and Hadfield), and so lead to the overestimation of the evolutionary potential of a trait. 

However, when the traits underlying the maternal effects have a genetic basis (i.e. maternal *genetic* effects, $V_{Mg}$), then the response to selection of these traits is no longer predicted by $V_A$ alone, with the 'total' $V_A$ being calculated as 
$$
V_A + 3/2 COV_{A,Mg} + 1/2 V_{Mg}
$$
(Willham, 1963;1972), with $V_{Mg}$ and $COV_{A,Mg}$ referring to maternal genetic variance and direct-maternal genetic covariance, respectively.
<!-- assuming no selection on parental performance (cheveraud/hadfield) -->
 From the few studies that have estimated maternal genetic effects, it is common for this maternal variation to have a considerable genetic component (Table 1), which is consistent with studies demonstrating genetic variation in parental care behaviours (REFS). Studies from livestock also show that direct and maternal genetic effects likely negatively covary (Wilson and Reale), although this correlation is probably small. Consistent with this, although estimated with considerable uncertainty, the point estimates these covariances in the wild are, with one exception, all negative. Not incorporating maternal genetic effects and any direct-maternal genetic covariance will therefore further bias the estimation of the evolutionary potential of a trait.

Although many studies have estimated maternal effects (moore et al 2019 collated 770 estimates from 116 studies), by far the majority of these have not modelled maternal *genetic* effects (only 2.2% of moore et als estimates). Typically maternal effects are estimated by including maternal ID as random term in an animal model (maternal environment effect). This disparity is likely due to three causes: 
first due to data constraints - estimating maternal genetic effects requires substantially more data, over more generations. 
Second, in taxa with biparental care, maternal variance per se if often not partitioned out, rather a brood variance, which included maternal and paternal contributions. 
Third, we believe there is a common assumption that any maternal genetic variation is captured by these maternal environment effects, in the same way that modelling individual ID captures permanent environment and additive genetic effects (for example Moore et al 2019 pool $V_Me$ and $V_Mg$ to get a total measure of maternal variance to compare with models that modelled only $V_Me$). 
However, a previous simulation study (Clements et al 2001) showed that not accounting for these maternal genetic effects can upwardly bias estimates of $V_A$ even when $V_Me$ is modelled, which has been corroborated in empirical studies from wild populations (table 1).

Although this effect has been noted before, we do not know systematically to what extent estimates of $V_A$ are affected, or how pedigree structure influences this.
The previous simulation work (Clements et al) is very animal breeding based, and focusses on breeding designs that are not very realistic to natural populations.
...
Models kruuk and wilson showed that the inflation of Va and underestimation of total Vm, but equally the underestimation of total h2 (wilson)
...
look at two very different 'representative systems' - deer and tits. Low immigration (male baised?), high polygamy, low number of offspring versus  high female biased immigration, low polygamy and large number of offspring. 
...
One reason that exclusively maternal genetic  mammal and plants  is that in many bird species the father also provides post-natal care. 
Captured often as nest effects
What impact to paternal effects have
Less confounding between father and genetic effects in genetic pedigrees


Previous work on the impact of social versus genetic pedigrees has focussed on the estimation of of h2 alone (but see garant paper of common environment). Social versus genetic pedigrees will have a clear impact on the estimation of parental effects, as genetic and social relationships become confounded, specifically the social and genetic father. 
more full sibs, so resemblence between sibs seems genetic not maternal (think this is right)
but also resemble their mother more than their father?
maybe it depends on the mate switching between broods? within a brood they will 
maybe it depnds on whther the info is coming from parent offspring or sibs - if parent offspring, then it might inflate maternal effects as the resemblance to their father decreases, but resemblence within a mother increases?? If info from sibs, then increases Va??
...
Many classic papers on estimating maternal effects look at the covariance between differently related individuals. Different individuals share different amount of direct genetic variance, maternal and maternal and genetic covariance (table 2). Only maternal siblings (full siblings and maternal half-siblings) share maternal environmental effects, but many other individuals related through their mothers will share some level of maternal genetic effects. Take a two cousins related through their mothers being full siblings - as their mothers are related, they will experience 
meaning that the covariance of two individual related in this way will contain some maternal genetic variance.
Direct additive genetic variance is estimated through these comparisons, and in a model with only maternal environment  effects, is separated from maternal effects through comparison of maternal siblings with other individuals. As some of these other individuals will share maternal genetic and direct-maternal covariance, the estimates of Va will include 
In this way we can see that the extend to which Va is biased will depend on the structure of the pedigree, and the degree to which the non-maternal sibling relationships are dominated by other maternal relationships which share Vmg
