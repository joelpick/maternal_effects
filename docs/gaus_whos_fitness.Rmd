...
Although many studies have estimated maternal effects (moore et al 2019 collated 770 estimates from 116 studies), by far the majority of these have not modelled maternal *genetic* effects (only 2.2% of moore et als estimates). Typically maternal effects are estimated by including maternal ID as random term in an animal model (maternal environment effect). This disparity is likely due to two causes: first due to data constraints - estimating maternal genetic effects requires substantially more data, over more generations. Second, we believe there is a common assumption that any maternal genetic variation is captured by these maternal environment effects, in the same way that modelling individual ID captures permanent environment and additive genetic effects (for example Moore et al 2019 pool $V_Me$ and $V_Mg$ to get a total measure of maternal variance to compare with models that modelled only $V_Me$). However, a previous simulation study (Clements et al 2001) showed that not accounting for these maternal genetic effects can upwardly bias estimates of $V_A$ even when $V_Me$ is modelled, which has been corroborated in empirical studies from wild populations (e.g. Wilson et al 2005 Soay Sheep, Kruuk and Hadfield 2007 Red Deer
<!-- mcfarlane? -->
).
